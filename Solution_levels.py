...
We’re going to define a few useful methods for you: - getNumberBetween(prompt, min, max)) repeatedly asks the user for a number between min and max with the prompt prompt - spinWheel() simulates spinning the wheel and returns a dictionary with a random prize - getRandomCategoryAndPhrase() returns a tuple with a random category and phrase for players to guess - obscurePhrase(phrase, guessed) returns a tuple with a random category and phrase for players to guess
